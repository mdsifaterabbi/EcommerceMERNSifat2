...
const HomePage = () => {
    ...
    const getAllCategory = async (req, res) => {
        try {
            const { data } = await axios.get(`${import.meta.env.VITE_REACT_API}/api/v1/category/get-category`);
            if (data?.success) {
    ...
    const getAllProducts = async () => {
        try {
            ...
            console.log(error);
            toast.error("Error occurred in getAllProducts function in HomePage.jsx page");
        }
    }
        ...
        <Layout title={'All Products'}>
            <div className="flex flex-col sm:flex-row flex-wrap min-h-[70vh]">
                <div className="basis-1/1 sm:basis-1/4 text-center border border-black">
                    ...
                    <div className="flex flex-row flex-wrap px-[20px] pt-[20px]">
                        {
                            categories?.map((c) => (
                                <div key={c._id}>
                                    <Checkbox key={filterKey} onChange={(e) => handleCategoryFilter(e.target.checked, c._id)} >
                                        {
                                            c.name
                                        }
                                    </Checkbox>
                    ...
                    <div className="flex flex-row flex-wrap px-[20px] pt-[20px]">
                        <Radio.Group key={filterKey} onChange={(e) => setRadio(e.target.value)}>
                            {
                                Prices?.map((p) => (
                    ...
                    <div className="flex flex-row flex-wrap px-[20px] pt-[20px]">
                        <button className="btn btn-outline" onClick={() => resetFilter()}>RESET FILTER</button>
                    </div>
